...
class HybridDataSource extends DataSource {
  ...
  fetch (url) {
    this.fetchCount += 1
    console.log('fetch count =', this.fetchCount)
    let p = fetch(url)
      .then(response => {
  ...
  // [{ name }]
  xetGenomes () {
    // let url = 'http://proto.informatics.jax.org/prototypes/mgv-data/genomes.json'
    let url = '/mgv-data/genomes.json'
  ...
  // [{ name, length }]
  getChromosomes (g) {
    if (!g.name) u.fail('getChromosomes: no genome name')
    let q = `
      <query
        model="genomic"
  ...
  getFeatures (g, c) {
    if (!g.name || !c.name) u.fail('getFeatures: genome or chromosome name')
    //
    let q = `
      <query
      model="genomic"
  ...
  getModels (g, c, s, e) {
    if (!g.name || !c.name || !s || !e) u.fail(`getModels: bad args: ${g.name} ${c.name} ${s} ${e}`)
    let q = `
    <query
    model="genomic"
  ...
  // { g, c, s, e, sequence }
  getSequence (g, c, s, e) {
    if (!g.name || !c.name || !s || !e) u.fail('getSequence: bad arguments: ${g.name} ${c.name} ${s} ${e}')
    let q = `
      <query model="genomic"
